...
{
    public partial class IntemBB : System.Web.UI.Page
    {
        ...
        protected void btnIntem_Click(object sender, EventArgs e)
        {
            ...
            if (rdMay2.Checked == true)
            {
                ...
                if (txtTGSX.Text == "")
                {
                    ...
                    string insert33 = "INSERT INTO [BB].[dbo].[TemBarcodeHC] (Plan_Id, Prd_Date, End_Date, Recipe_ID, Equip_Code, Weight, BB_machno, Ursno, Print_dat, print_num, Somecon)"
                            + "VALUES ('" + plan_id + "', '" + txtSolo.Text.Trim().ToString().Replace("-", "") + "', '" + txtNHL.Text.Trim().ToString().Replace("-", "") + "', '" + drChonmakeo.SelectedItem.Text + "', '" + maybb + "', '" + weight + "' , '" + maybb + "', '" + Session["username"].ToString() + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + txtSoMeSX.Text.Trim() + "', '" + somecon + "');";

                    cnnstr.TryGetValue("33", out string cnn33sql);
                    bool sql33 = SQLpro.ExecuteNonQuery(insert33, cnn33sql);
                    txtSoMeSX.Text = somecon;
                    txtPlanNum.Text = somecon;
                    if (PingMachine(maybb))
                    {
                        cnnstr.TryGetValue(maybb, out string cnnMay);
                        string InserSQL1 = "Insert into [AutoSmall_ScanCode] values ('" + plan_id + "','" + DateTime.Now.ToString("yyyyMMdd") + "','" + DateTime.Now.AddDays(7).ToString("yyyyMMdd") + "','" + drChonmakeo.SelectedItem.Text + "','"+ maybb + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + weight + "',0)";

                        bool a = SQLpro.ExecuteNonQuery(InserSQL1,cnnMay);
                ...
                else
                {
                    ...
                    if (dtrecipe.Rows.Count > 0)
                    {
                        ...
                        DataTable dt = SQL.ExecuteQuery15(sqltotalweight);
                        ...
                        string checkgdaxuat = "  SELECT Weight AS sokg FROM [BB].[dbo].[TemBarcodeHC] WHERE Plan_Id = '" + plan_id + "' AND BB_machno = '" + maybb + "' order  by Print_dat desc ;";
                        DataTable dtSkgx = SQL.ExecuteQuery33BB(checkgdaxuat);
                        if (dtSkgx.Rows.Count > 0)
                        {
                            sokgxuat = decimal.Parse(dtSkgx.Rows[0][0].ToString().Trim());
                        }
                        ...
                        {
                            cnnstr.TryGetValue(maybb, out string cnnMay);
                            string InserSQL1 = "Insert into [AutoSmall_ScanCode] values ('" + plan_id + "','" + txtSolo.Text.Trim().ToString().Replace("-", "") + "','" + txtNHL.Text.Trim().ToString().Replace("-", "") + "','" + drChonmakeo.SelectedItem.Text + "','"+ maybb + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + weight + "',0)";
            ...
            if (rdMay1.Checked == true)
            {
                ...
                else
                {
                    ...
                    if (dtrecipe.Rows.Count > 0)
                    {
                        ...
                        DataTable dt = SQL.ExecuteQuery16(sqltotalweight);
                ...
                else
                {
                    ...
                    if (dtrecipe.Rows.Count > 0)
                    {
                        if (dtrecipe.Rows.Count > 1)
                        {
                            ...
                            plan_id = dtrecipe.Rows[0][0].ToString();
                        int plan_num = 0;
                        string sqltotalweight = "SELECT Total_Weight FROM [CWSS_S7].[dbo].[Pmt_recipe] where  recipe_name ='" + drChonmakeo.SelectedItem.Text + "'" + "and Equip_Code ='02'" ;
                        DataTable dt = SQL.ExecuteQuery17(sqltotalweight);
            ...
            if (rdMay04.Checked == true)
            {
                ...
                else
                {
                    ...
                    if (dtrecipe.Rows.Count > 0)
                    {
                        ...
                        DataTable dt = SQL.ExecuteQuery18(sqltotalweight);
                        if (dt.Rows.Count > 0)
                        {
                            total_weight = Convert.ToDecimal(dt.Rows[0]["Total_Weight"].ToString());
                        }else
            ...
            //    DateTime expiryTime = DateTime.ParseExact(txtNHL.Text.Trim(), "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
            //    DateTime updateby = DateTime.ParseExact(txtSolo.Text.Trim(), "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
        ...
        // Hàm ánh xạ mã máy thành địa chỉ IP
